                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data">
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="content" name="content" rows="15" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>{{ old('content', $post->content) }}</textarea>
                        ...
                        <div class="mt-4">
                            ...
                            @if ($post->image)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/posts/' . $post->image) }}" alt="Current image" class="max-w-xs h-auto rounded">
                                    <p class="text-sm text-gray-500 mt-1">Current image</p>
                                </div>
                            @endif
                            <input id="image" class="block mt-1 w-full" type="file" name="image" accept="image/*" />
                            <x-input-error :messages="$errors->get('image')" class="mt-2" />
                            <p class="mt-1 text-sm text-gray-500">Max 2MB. Allowed formats: JPEG, PNG, JPG, GIF. Leave empty to keep current image.</p>
                        </div>
